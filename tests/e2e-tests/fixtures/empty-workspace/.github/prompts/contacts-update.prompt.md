---
description: Use WorkIQ to discover an org subtree starting from a person and route everyone into Peers.md (managers), Colleagues.md (individual contributors), and Management.md (the person's management chain), capturing EmployeeID for everyone. Supports refreshing or fully replacing the lists after team changes.
name: "Contacts Update"
agent: "agent"
tools: [read/readFile, agent, edit/editFiles]
---

Use **WorkIQ** (the `ask_work_iq` tool or the `WorkIQ` agent) to discover an org subtree starting from a person, then write people into three contact files using each file's own schema:
...
1. **Peers** — the person reports **directly** to **M** and is **confirmed to be a manager** (has one or more direct reports).
2. **Management** — the person is in **P's management chain** (i.e. **M**, **GM**, and any manager above them up the chain).
3. **Colleagues** — everyone else in the enumeration.
...
Each contact is an H1 heading equal to the person's **alias**, followed by the property bullets in the order shown in that file's existing schema. Capture **`EmployeeID` for everyone** in all three files.
...
Match each file's existing property order and which properties it carries. **Leave any property you cannot find empty** (keep the bullet, with no value after the colon).
...
**Preservation rule (both modes):** Never overwrite a **non-empty** value that WorkIQ does not own. This protects:
...
- **`replace`**: Same as refresh, **plus** delete entries for anyone in the three files who is no longer in scope, and **move** anyone whose routing changed to the correct file (e.g. a Colleague who became a manager moves to Peers).
...
Report a concise summary grouped **per file** (Peers / Colleagues / Management). For each person list **only the property names** that were set (never the values):
