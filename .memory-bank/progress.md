...
- **Test coverage**: 1225 unit tests passing (74 test files), 7 E2E tests passing. Includes 25 unit tests for `BlueprintDecorationProvider` and 3 contract tests for `package.json` command declarations.
...
- **Custom Todo Editor**: `CustomTextEditorProvider` for `*.todo.md` files — visual task board with drag-and-drop reordering, checkbox completion/un-completion (optimistic UI), add/edit task popups, source file navigation, and collapsible completed section. Feature-gated via `taskCollector` toggle. Webview powered by `markdown-it` for task body rendering. Source file write-back for collected tasks. Performance-optimized: lazy MarkdownIt init, markdown render cache, fingerprint-based update skipping, external CSS bundle with skeleton placeholder, incremental DOM reconciliation in webview, cached workspace root and source-by-body map, debounced text-change propagation (80ms). Keyboard shortcuts for bold/italic/link editing. Link autocompletion for local files. Documented in user guide. Create-linked-file from link autocomplete dropdown. File/folder rename propagation across all .md files.
...
- **Code quality pass**: Shared utilities extracted (`utils/webview.ts`, `utils/markdownCheckbox.ts`, `utils/jsonCompletionHelpers.ts`, `utils/dateUtils.ts`, `utils/encoding.ts`, `utils/filesystem.ts`, `utils/markdown.ts`, `utils/regex.ts`). Large files decomposed into focused modules: `todoEditorProvider` split into provider + `todoEditorMessageHandler`, `taskCollectorFeature` split into feature + `taskCollectorPathResolver` + `taskCollectorTransformer`, contacts `main.ts` decomposed into 12+ focused webview modules, `contactsViewProvider` split into provider + `contactsViewMapping` + `contactsViewHtml` + `contactFileLoader` + `contactMutations`. `featureSetup.ts` extracted with `createToggle()` utility. Magic numbers replaced with named constants. File-watcher pattern differences documented.
...
## Current Status
Version 0.0.1, not publicly released. All features implemented. Full telemetry coverage. `ReporterTelemetrySender` adapter enables production AppInsights telemetry. Istanbul patch persisted via `patch-package`. 1225 unit tests passing, 74 test files.
