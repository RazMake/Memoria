# Getting Started with Memoria

**Memoria** is a VS Code extension that scaffolds a notebook inside a workspace.  
It creates a folder structure, deploys content that is considered useful for the specific type of notebook (_or knowledge base_) and enables features specific to the selected customization package, called **blueprints**.  

To get started, first thing you need to do is to initialize your workspace, by following the steps below.

## Initialize Your Workspace

1. Open the **Command Palette** (`Ctrl+Shift+P`)
2. Run [`Memoria: Initialize workspace`](commands/initialize-workspace.md) command
3. Choose a blueprint from the [available](blueprints/index.md) list  
   Note: _The extension supports both normal and multi-root workspaces_
4. After the workspace is initialized, additional [commands](commands/index.md) will be available

## Initialization walkthrough

![Initialize workspace](media/initialize-workspace.gif)

## Need Help?

Run [`Memoria: Open User Guide`](commands/open-user-guide.md) from the Command Palette at any time to browse the built-in documentation.

## Next Steps

- Explore the [blueprints](blueprints/index.md)
- Learn about all available [commands](commands/index.md)
- Learn more about [features](features/index.md) packaged with the blueprints
- Read the [FAQ & Troubleshooting](faq.md)
