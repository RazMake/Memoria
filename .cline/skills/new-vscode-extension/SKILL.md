...
- **Telemetry**: `@vscode/extension-telemetry` with local console fallback (no AppInsights key required for development)
...
export default defineConfig({
    test: {
        ...
        coverage: {
            ...
            // AI-AGENT: When the first testable module (non-extension.ts source file
            // beyond telemetry.ts) is added to this extension, uncomment the thresholds
            // below and ensure coverage meets 85%.
            // thresholds: {
            //     statements: 85,
            //     branches: 85,
            //     functions: 85,
...
/**
 * A TelemetrySender that writes events to a VS Code OutputChannel
 * for local observability when no Application Insights key is configured.
 */
 ...
 *
 * - With a connection string: returns a TelemetryReporter that sends to Application Insights.
...
): TelemetryReporter | vscode.TelemetryLogger {
    if (connectionString) {
...
1. Import `createTelemetry` from `./telemetry`.
2. `activate(context: vscode.ExtensionContext)` initializes telemetry and registers each command via `context.subscriptions.push(vscode.commands.registerCommand(...))`.
3. Each handler shows a placeholder info message: `vscode.window.showInformationMessage("<displayName>: <title>")`.
4. Export an empty `deactivate(): void`.
...
// Mock telemetry module to avoid AppInsights import in unit tests
...
```ts
import { describe, it, expect, vi, beforeEach } from "vitest";
import { ConsoleTelemetrySender } from "../src/telemetry";
...
describe("createTelemetry", () => {
    ...
    it("returns TelemetryReporter when connection string is provided", async () => {
        const { createTelemetry } = await import("../src/telemetry");
        const TelemetryReporter = (await import("@vscode/extension-telemetry")).default;
        const context = { subscriptions: [] } as any;

        const result = createTelemetry(context, "InstrumentationKey=test-key");
        expect(TelemetryReporter).toHaveBeenCalledWith("InstrumentationKey=test-key");
